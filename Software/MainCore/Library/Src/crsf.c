...
char flightMode[CRSF_FLGHT_MODE_LEN];
...
////	elrs_info.bad_pkts = buffer[3];                     // bad packet rate (should be 0)
////	elrs_info.good_pkts = (buffer[4] << 8) + buffer[5];// good packet rate (configured rate)
////	elrs_info.flags = buffer[6];
////	uint8_t len = strlen((const char *)buffer + 6);
////	if(len > CRSF_MAX_NAME_LEN)
////		printf("Elrs_info.flag_info name err\n");
////	else
////		strlcpy(elrs_info.flag_info, (const char *)&buffer[7], len); // null-terminated text of flags
...
uint8_t ParamsLoaded(uint8_t addr)
{
	...
	if(addr == ELRS_ADDRESS)	//Transmitter or receiver params?
		a = 0;
	else
		a = 1;
        ...
        if (CalculatedCRC == SerialInBuffer[SerialInPacketPtr - 1])
        {
          ...
          if (id == CRSF_FRAMETYPE_BATTERY_SENSOR)
          {
            if (getCrossfireTelemetryValue(3, &value, 2))
            {
					...
					if(id == TYPE_FLIGHT_MODE)
					{
//						uint8_t len = strlen((const char*)SerialInBuffer+3);
//						if(len > 0)
//						{
//							memset((void *)elrs_info.flag_info, 0x00, sizeof(elrs_info.flag_info));
//							strlcpy(elrs_info.flag_info, (const char*)SerialInBuffer+3, len+1);
//						}
//						else
//						{
//							strlcpy(elrs_info.flag_info, "UNKNOWN", 7);
//						}
						strlcpy(flightMode, (const char*)SerialInBuffer+3, CRSF_FLGHT_MODE_LEN-1);
					}
