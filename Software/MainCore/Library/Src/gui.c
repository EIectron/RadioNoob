...
void DrawTopBar(void)
{
	...
	GFXDisplayPutImage(BatteryPaddingX-80,BatteryPaddingY, &usb_logo, false);
	...
	sprintf(text, "%d", calc_battery_percentage(vBat));
	GFXDisplayPutString(BatteryPaddingX-40, BatteryPaddingY, &fontConsolas24h, text, WHITE, TRANSPARENT);
...
void DrawJoystick(void)
{
		...
		rc_values[THROTTLE] = mapJoystickValues(myRCvalues.channelValues[THROTTLE], myTableParams.channels[THROTTLE].min, 
																																myTableParams.channels[THROTTLE].mid, 
																																myTableParams.channels[THROTTLE].max, 1);
		rc_values[YAW] = mapJoystickValues(myRCvalues.channelValues[YAW], myTableParams.channels[YAW].min, 
																																myTableParams.channels[YAW].mid, 
																																myTableParams.channels[YAW].max, 1);
		rc_values[PITCH] = mapJoystickValues(myRCvalues.channelValues[PITCH], myTableParams.channels[PITCH].min, 
																																myTableParams.channels[PITCH].mid, 
																																myTableParams.channels[PITCH].max, 1);
		rc_values[ROLL] = mapJoystickValues(myRCvalues.channelValues[ROLL], myTableParams.channels[ROLL].min, 
																																myTableParams.channels[ROLL].mid, 
																																myTableParams.channels[ROLL].max, 1);
		
		y1 = map(rc_values[THROTTLE], 850, 2100, 220, 100);
		y2 = map(rc_values[PITCH], 850, 2100, 220, 100);
		x1 = map(rc_values[YAW], 900, 2100, 170, 50);
		x2 = map(rc_values[ROLL], 900, 2100, 340, 220);
...
void DrawChannels(void)
{
	...
	rc_values[0] = mapJoystickCRSFValues(myRCvalues.channelValues[0], myTableParams.channels[0].min, 
	...
	rc_values[1] = mapJoystickCRSFValues(myRCvalues.channelValues[1], myTableParams.channels[1].min, 
	...
	rc_values[2] = mapJoystickCRSFValues(myRCvalues.channelValues[2], myTableParams.channels[2].min, 
	...
	rc_values[3] = mapJoystickCRSFValues(myRCvalues.channelValues[3], myTableParams.channels[3].min, 
	...
	rc_values[4] = mapJoystickCRSFValues(myRCvalues.channelValues[4], myTableParams.channels[4].min, 
	...
	rc_values[5] = mapJoystickCRSFValues(myRCvalues.channelValues[5], myTableParams.channels[5].min, 
	...
	rc_values[6] = mapJoystickCRSFValues(myRCvalues.channelValues[6], myTableParams.channels[6].min, 
	...
	rc_values[7] = mapJoystickCRSFValues(myRCvalues.channelValues[7], myTableParams.channels[7].min, 
		...
		uint16_t x[8];
		x[0] = map(rc_values[0], CHANNEL_MIN, CHANNEL_MAX, 100, 300);
		x[1] = map(rc_values[1], CHANNEL_MIN, CHANNEL_MAX, 100, 300);
		x[3] = map(rc_values[3], CHANNEL_MIN, CHANNEL_MAX, 100, 300);
		x[2] = map(rc_values[2], CHANNEL_MIN, CHANNEL_MAX, 100, 300);
		x[4] = map(rc_values[4], CHANNEL_MIN, CHANNEL_MAX, 100, 300);
		x[5] = map(rc_values[5], CHANNEL_MIN, CHANNEL_MAX, 100, 300);
		x[6] = map(rc_values[6], CHANNEL_MIN, CHANNEL_MAX, 100, 300);
		x[7] = map(rc_values[7], CHANNEL_MIN, CHANNEL_MAX, 100, 300);
...
void DrawMainScreen(void)
{
	...
	{
			sprintf(lcd_txt, "%02d:%02d", (((millis() / 1000) - flightTimeStart) / 60), (((millis() / 1000) - flightTimeStart) % 60));
			GFXDisplayPutText(25, 50, &Arcade28, lcd_txt, BLACK);
	}

	

	
	if(strlen(flightMode) > 0)
		GFXDisplayPutText(25, 80, &Arcade28, flightMode, BLACK);
	else
		GFXDisplayPutText(25, 80, &Arcade28, "--", BLACK);
	...
	sprintf(lcd_txt, "%.1fV", ((float)Battery.voltage / 10));
	GFXDisplayPutString(230, 160, &fontConsolas24h, lcd_txt, BLACK, WHITE);
	
	sprintf(lcd_txt, "%.1fV", vBat);
	GFXDisplayPutString(290, 160, &fontConsolas24h, lcd_txt, BLACK, WHITE);
	
	sprintf(lcd_txt, "%c%d", '%', LinkStatistics.uplink_Link_quality);
	GFXDisplayPutString(230, 190, &fontConsolas24h, lcd_txt, BLACK, WHITE);
	
	sprintf(lcd_txt, "%dmW", LinkStatistics.uplink_TX_Power);
	GFXDisplayPutString(290, 190, &fontConsolas24h, lcd_txt, BLACK, WHITE);
...
//															Bad_Good,
	...
	if(guiMenu != _guiMenu)
	{
		...
		QEI_SET_CNT_MAX(QEI0, itemCount);
		QEI_Start(QEI0);
		next_param = ParamsLoaded(ELRS_ADDRESS);
		command_stat = false;
	}
	
	if(ParamsLoaded(ELRS_ADDRESS) <= crsf_devices[0].number_of_params)
	{
...
//				GFXDisplayPutString(90, 120, &fontConsolas24h, "Binding...", BLACK, WHITE);
				guiMenu = BindScreen;
				next_param = Bind;
				crsf_params[Bind].status = PROGRESS;
				crsf_params[Bind].current_value = 1;
			}
...
void DrawTxPowerScreen(void)
{
	...
	if(guiMenu != _guiMenu)
	{
		...
		if(crsf_params[param_list[selectIndex]].id == 0x00)
			return;
	...
	if(ENCODER_SW == 0)
	{
		...
		if(selectIndex > -1)
		{
			if(crsf_params[param_list[selectIndex]].status != indexValue) 		// if value is change? update new value.
			{
				CRSF_changeParam(param_list[selectIndex], indexValue);					// CRSF value update
//				printf("[Change param] p: %d, val: %d\n", param_list[selectIndex], indexValue); // For debug
				crsf_params[TX_Power].id = 0x00;		// to change main screen tittle (tx power)
				crsf_params[param_list[selectIndex]].id = 0x00;
...
void DrawBindScreen(void)
{
	
	if(guiMenu != _guiMenu)
	{
		_guiMenu = guiMenu;
		if(crsf_params[Bind].current_value == READY)
			GFXDisplayPutString(90, 120, &fontConsolas24h, "Binding...         ", BLACK, WHITE);
		else
			GFXDisplayPutString(90, 120, &fontConsolas24h, crsf_params[Bind].s.info, BLACK, WHITE);
		GFXDisplayUpdate();
	}
	
	if(crsf_params[Bind].u.status != crsf_params[Bind].status)
	{
		next_param = Bind;
		next_chunk = crsf_params[Bind].current_value;
		command_stat = true;
	}
	else
	{
	...
	if(END_SW_PIN == 0)
	{
		Delay(50);
		while(END_SW_PIN == 0);
		if(crsf_params[Bind].u.status == READY)
		{
			if(selectIndex >= 0)
			{
				guiMenu = ExpressLRSScreen;
		...
		{
			crsf_params[Bind].status = READY;
			crsf_params[Bind].current_value = READY;
		}
	}
}
...
//		GFXDisplayPutString(15, 34, &fontConsolas24h, "Welcome to, RadiolinkAT9C", BLACK, WHITE);		
		...
		GFXDisplayPutText(330, 210, &abeatbykai24, VERSION, WHITE);
		GFXDisplayUpdate();
		Delay(1000);
}
